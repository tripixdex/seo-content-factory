...
Universal RAG Copilot — локальный демонстрационный проект для ответов по документам с проверкой достаточности доказательств.

Главная идея проекта: система должна отвечать только тогда, когда в загруженных материалах есть подходящие фрагменты. Если подтверждений недостаточно, она должна честно отказаться от ответа, а не выдумывать.

## Что показывает проект

- Разбиение документов на фрагменты.

- Лексический поиск по локальным материалам.

- Оценку найденных фрагментов по релевантности.

- Решение: можно отвечать или доказательств недостаточно.

- Два явных состояния результата: `answerable` и `not_enough_evidence`.

- Запуск через командную строку, API и простой веб-интерфейс.

- Локальные проверочные сценарии и отчёты.

## Важное ограничение

В текущей версии проект не является полноценным ИИ-ассистентом с подключенной языковой моделью. Он демонстрирует основу доказательного ответа: поиск фрагментов, оценку достаточности данных и безопасный отказ, если ответа нет в материалах.

В проекте также не используются векторные представления текста, переранжирование и гибридный поиск. Поиск сделан лексическим, чтобы поведение было простым, локальным и проверяемым.

## Почему это важно для бизнеса

Для внутренних инструментов опасно, когда система уверенно отвечает без опоры на данные. Этот проект показывает мой подход к безопасной автоматизации: сначала доказательства, потом ответ. Если доказательств нет — лучше честно сказать “данных недостаточно”, чем создать риск неправильного решения.
